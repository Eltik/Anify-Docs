...
<Tabs items={["Parameters", "Query", "Post"]}>
    <Tab>
        <>
            ```
            https://api.anify.tv/search/{type}/{query}/{page}/{perPage}/{sort}/{sortDirection}
            ```
        </>
...
|   apikey  | string | Whitelisted API key |    No    |  `""`   |
        ...
        <Tabs items={["JavaScript", "Python"]}>
            <Tab>
                <>
                    ...
                    const url = `https://api.anify.tv/search/anime/mushoku%20tensei/1/20/averagePopularity/ASC`;
                    const data = async () => {
                        try {
            ...
            <Tab>
                <>
                    ...
                    # Using the example query "mushoku tensei"
                    url = "https://api.anify.tv/search/anime/mushoku%20tensei/1/20/averagePopularity/ASC"
    ...
    <Tab>
        <>
            ```
            https://api.anify.tv/search?type={type}&query={query}&page={page}&perPage={perPage}&sort={sort}&sortDirection={sortDirection}
            ```
        </>
| Parameter |  Type  | Description                                                       | Required? | Default |
| :-------: | :----: | ----------------------------------------------------------------- | :-------: | :-----: |
        ...
        <Tabs items={["JavaScript", "Python"]}>
            <Tab>
                <>
                    ...
                    // Using the example query "mushoku tensei"
                    const url = `https://api.anify.tv/search?type=anime&query=mushoku%20tensei&page=1&perPage=20&sort=averagePopularity&sortDirection=ASC`;
                    const data = async () => {
                        try {
                            const { data } = await axios.get(url);
                            return data;
            ...
            <Tab>
                <>
                    ...
                    # Using the example query "mushoku tensei"
                    url = "https://api.anify.tv/search?type=anime&query=mushoku%20tensei&page=1&perPage=20&sort=averagePopularity&sortDirection=ASC"
                    response = requests.get(url)
                    data = response.json()
        ...
        <Tabs items={["JavaScript", "Python"]}>
            ...
            <Tab>
                <>
                    ...
                    response = requests.post(url, json={'type': "anime", 'query': "mushoku tensei", 'page': 1, 'perPage': 20, 'sort': "averagePopularity", 'sortDirection': "ASC"})
                    data = response.json()
...
<Tabs items={["200 OK", "400 Bad Request", "500 Internal Server Error"]}>
    ...
    <Tab>
        **MIME Type:** `application/json`<br />
        **Reason:** `Occurs when the the "type" parameter isn't given/when an invalid "type" parameter is given, when no "query" parameter is given, when an invalid sort type is provided, or when an invalid sort direction is provided.`
        ```json copy=false
