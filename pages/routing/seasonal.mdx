...
# Seasonal
Used for querying AniList's seasonal route and returning media. This route also inserts media if some of AniList's media doesn't exist in the database like the `search` route. This route will add media to the mapping queue to be mapped if there are no results found (which is why a high rate limit is necessary). Technical details regarding the usage of the search function for anime, manga, and novels can be found below. Example code is provided for both JavaScript and Python, along with a response schema.

## Rate Limits
The search route has a rate limit of 50 requests per minute unless your API key is whitelisted.
